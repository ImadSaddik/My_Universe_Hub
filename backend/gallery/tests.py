...
class GetTodayExistingPictureTestCase(APITestCase):
    def setUp(self):
        ...
        self.gallery = Gallery.objects.create(
            date=self.today, title="test image", explanation="Explanation", image_url="https://exmaple.com/image.jpg"
        )
...
class LikeImageTestCase(APITestCase):
    def setUp(self):
        self.today = datetime.now().today().date()
        self.gallery = Gallery.objects.create(
            date=self.today, title="test image", explanation="Explanation", image_url="https://example.com/image.jpg"
        )
        self.user = User.objects.create_user(username="testuser", password="testpass")

    def test_like_image(self):
        url = reverse("like_image")
        data = {"date": str(self.today), "username": "testuser"}
    ...
    def test_like_image_invalid_date(self):
        url = reverse("like_image")
        data = {"date": "2022-01-01", "username": "testuser"}
        response = self.client.post(url, json.dumps(data), content_type="application/json")
    ...
    def test_like_image_invalid_user(self):
        url = reverse("like_image")
        data = {"date": str(self.today), "username": "invaliduser"}
        response = self.client.post(url, json.dumps(data), content_type="application/json")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.content.decode("utf-8"), "Image not found!")
